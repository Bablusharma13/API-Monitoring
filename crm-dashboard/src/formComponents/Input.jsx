...
const Input = React.forwardRef(({
  ...
  return (
    <div className="w-full">
      ...
      <div className={`relative border rounded-lg px-3 pt-2 pb-2 transition-all
        ${disabled ? "bg-gray-50" : "bg-white"}
        ${styles.wrapper}`}
      >
        {/* Floating label — sits on the border */}
        {label && (
          <span className={`absolute -top-[9px] left-2.5 px-1
            flex items-center gap-1 text-[11px] leading-none
            ${styles.label}
            ${state === "error" ? "bg-red-50" :
              state === "success" ? "bg-green-50" :
              state === "disabled" ? "bg-gray-50" : "bg-white"}`}
          >
            {icon && (
        ...
        <input
          ...
          onBlur={onBlur}
          disabled={disabled}
          min={!allowNegative && type === "number" ? (minValue ?? 0) : minValue}
          max={maxValue}
          onKeyDown={(e) => {
            preventNegative(e);
